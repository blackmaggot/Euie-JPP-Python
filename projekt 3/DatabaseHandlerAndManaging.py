...
class DatabaseHandlerAndManaging(object):
    ...
    def getFromDb(self):
        with self.con:
            ...
            p = cur.execute("select * from students").fetchall()
            firstNamesList = []
            for i in range(0, len(p)):
                # encodedString = str(row).encode("utf-8")
                encodedStringIndex = str(p[i][1]).encode("utf-8")
                encodedStringName = str(p[i][2]).encode("utf-8")
                encodedStringSurename = str(p[i][3]).encode("utf-8")
                encodedStringPesel = str(p[i][4]).encode("utf-8")
